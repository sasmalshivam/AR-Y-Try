...
const TryOn = () => {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'fixed', top: 0, left: 0, background: '#000', zIndex: 9999 }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(to bottom, rgba(0,0,0,0.8), transparent)' }}>
      ...
      <div style={{ position: 'absolute', bottom: '100px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(20, 20, 20, 0.8)', backdropFilter: 'blur(15px)', borderRadius: '16px', padding: '1rem', display: 'flex', alignItems: 'center', gap: '1rem', width: '90%', maxWidth: '400px', zIndex: 10 }}>
        <div style={{ flex: 1 }}>
        ...
        <button style={{ padding: '0.8rem 1.5rem', fontWeight: 'bold', border: 'none', borderRadius: '10px', background: '#5C3317', color: '#FAF0E6', cursor: 'pointer' }}>
          Add to Cart
        </button>
      </div>

      <ProductSwitcher products={arProducts} currentProduct={currentProduct} onSelectProduct={setCurrentProduct} />
    </div>
  );
};
